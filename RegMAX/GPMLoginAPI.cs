...
namespace RegMAX
{
    public class GPMLoginAPI
    {
        ...
        {
            /*string url = _apiUrl + API_CREATE_PATH;
            Data profile = new Data
            {
                profile_name = name,
                group_name = group,
                raw_proxy = proxy,
                is_masked_font = fakeFont,
                is_noise_canvas = isNoiseCanvas,
                webrtc_mode = turnOnWebRTC ? 1 : 2
                // ... (Thêm các thuộc tính khác của đối tượng tại đây)
               string jsonPayload = JsonConvert.SerializeObject(profile);
            };*/
        ...
        private string httpPost(string url,string jsonPayload)
        {
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    // Định nghĩa nội dung HTTP với payload JSON
